...
(function() {
    ...
    function trackGamePlay() {
        // 获取当前游戏计数
        let gameCount = parseInt(localStorage.getItem('woodcat_game_count')) || 0;
        
        // 检查是否已经显示过PWA引导
        const pwaPromptShown = localStorage.getItem('woodcat_pwa_prompt_shown') === 'true';
        
        // 如果还没显示过引导，增加游戏计数
        if (!pwaPromptShown) {
            gameCount++;
            localStorage.setItem('woodcat_game_count', gameCount);
            console.log(`游戏计数: ${gameCount}`);
    ...
    function showPwaPrompt() {
        ...
        if (pwaModal) {
            ...
            // 绑定事件
            document.getElementById('pwa-yes').onclick = function() {
                pwaModal.style.display = 'none';
                showDeviceSelector();
            };
            
            document.getElementById('pwa-no').onclick = function() {
                pwaModal.style.display = 'none';
                // 标记为已显示，不再提示
                localStorage.setItem('woodcat_pwa_prompt_shown', 'true');
            };
    ...
    function showDeviceSelector() {
        ...
        if (deviceModal) {
            ...
            // 绑定事件
            document.getElementById('device-iphone').onclick = function() {
                deviceModal.style.display = 'none';
                showGuide('iphone');
            };
            
            document.getElementById('device-android').onclick = function() {
                deviceModal.style.display = 'none';
                showGuide('android');
            };
            
            document.getElementById('device-desktop').onclick = function() {
                deviceModal.style.display = 'none';
                showGuide('desktop');
            };
            
            document.getElementById('device-modal-close').onclick = function() {
                deviceModal.style.display = 'none';
                localStorage.setItem('woodcat_pwa_prompt_shown', 'true');
            };
    ...
    function showGuide(deviceType) {
        ...
        if (guideModal && guideContent) {
            ...
            switch(deviceType) {
                case 'iphone':
                    content = `
                        <h4 style="margin-top: 0;">iPhone/iPad 添加步骤：</h4>
                        <ol style="text-align: left; margin-top: 20px;">
                            <li>点击浏览器底部的 <strong>分享</strong> 按钮</li>
                            <li>在分享菜单中，向上滑动找到 <strong>添加到主屏幕</strong> 选项</li>
                            <li>点击 <strong>添加到主屏幕</strong></li>
                            <li>确认添加，然后点击 <strong>完成</strong></li>
                            <li>现在您可以在主屏幕上找到木头猫游戏合集的图标了！</li>
                        </ol>
                        <p style="margin-top: 20px; font-weight: bold;">提示：如果没有看到"添加到主屏幕"选项，请先点击"更多"选项。</p>
                    `;
                    break;
                case 'android':
                    content = `
                        <h4 style="margin-top: 0;">Android 添加步骤：</h4>
                        <ol style="text-align: left; margin-top: 20px;">
                            <li>点击浏览器右上角的 <strong>菜单</strong> 按钮（通常是三个点）</li>
                            <li>在菜单中选择 <strong>添加到主屏幕</strong> 或类似选项</li>
                            <li>确认添加，然后点击 <strong>确定</strong></li>
                            <li>现在您可以在主屏幕上找到木头猫游戏合集的图标了！</li>
                        </ol>
                        <p style="margin-top: 20px; font-weight: bold;">提示：不同Android浏览器的选项名称可能略有不同。</p>
                    `;
                    break;
                case 'desktop':
                    content = `
                        <h4 style="margin-top: 0;">桌面端添加步骤：</h4>
                        <p style="margin-top: 20px;">对于桌面端用户，您可以：</p>
                        <ol style="text-align: left; margin-top: 10px;">
                            <li><strong>Chrome浏览器：</strong>点击地址栏右侧的 <strong>安装</strong> 图标，然后点击 <strong>安装</strong></li>
                            <li><strong>Edge浏览器：</strong>点击地址栏右侧的 <strong>添加到应用</strong> 按钮，然后点击 <strong>添加</strong></li>
                            <li><strong>Firefox浏览器：</strong>点击地址栏左侧的 <strong>页面信息</strong> 图标，然后选择 <strong>添加到主屏幕</strong></li>
                        </ol>
                        <p style="margin-top: 20px; font-weight: bold;">提示：安装后，您可以在开始菜单或应用程序列表中找到木头猫游戏合集。</p>
                    `;
                    break;
            }
            ...
            // 绑定事件
            document.getElementById('guide-close').onclick = function() {
                guideModal.style.display = 'none';
                localStorage.setItem('woodcat_pwa_prompt_shown', 'true');
            };
            
            document.getElementById('guide-modal-close').onclick = function() {
                guideModal.style.display = 'none';
                localStorage.setItem('woodcat_pwa_prompt_shown', 'true');
            };
    ...
    // 修改游戏卡片点击事件，添加游戏计数
    const originalHandleGameCardClick = handleGameCardClick;
    handleGameCardClick = function(card, e) {
        // 先执行原始的点击处理
        originalHandleGameCardClick(card, e);
    ...
    function initPwaPrompt() {
        // 检查当前游戏计数
        const gameCount = parseInt(localStorage.getItem('woodcat_game_count')) || 0;
        const pwaPromptShown = localStorage.getItem('woodcat_pwa_prompt_shown') === 'true';
        
        console.log(`初始化PWA提示 - 游戏计数: ${gameCount}, 已显示: ${pwaPromptShown}`);
        
        // 如果已经达到计数但还没显示过引导，立即显示
        if (gameCount >= 2 && !pwaPromptShown) {
            showPwaPrompt();
        }
    }
